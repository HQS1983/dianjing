...
$(function(){
	...
	function bannerSwiper(){
		...
		for(var QQ=0; QQ<=3; QQ++ ){
			html += '<div class="swiper-slide"><div class="banner_con"><img src="img/index/banner'+(QQ+1)+'.jpg"/></div></div>';
		}
...
//		return currentdate;
	...
	function pClass(activeIndex){
		var html="",playArea="";
		for(var QQ=1; QQ<=activeIndex; QQ++ ){
			if (activeIndex == 17 ) {
				playArea="王者荣耀";
				$(".pullDown").attr("data-switch","0");
				$(".imgClass1").attr("src","img/allgame/1.png");
				$(".imgClass1,.spanClass").css("opacity","1");
...
//						'</a>';
				...
				$(".imgClass1").attr("src","");
				$(".imgClass1,.spanClass").css("opacity","0");
			}
		}
	...
	function EventList(activeIndex){
		...
		if (activeIndex == 17 || activeIndex == 21 || activeIndex == 25) {
			...
			$(".calendar_bottom .EventList").append(html2);
		}
	}
...
//			$("#arrangement .notHave").css("height","0");
			html2="";
			$("#arrangement .notHave").css({height:"0",opacity:"0"});
			$("#arrangement").append(html2+html);
		}else{
			$("#arrangement .notHave").css({height:"3rem",opacity:"1"});
			html="";
			$("#arrangement").append(html+html2);
...
//			speed:3000,
			slidesPerView : 7,
	        autoplayDisableOnInteraction : false,
	        initialSlide:strDate,//用来设定页面加载完成时，第几张图片先显示出来
    		centeredSlides: true,
				...
				if ( res.code == 0 ) {
					$.each(res.datas,function(i,item){
                        var html = '<div class="events shadow2 fix">'+
                                ...
                                '<p class="money apostrophe"><img src="img/icon_PopularEvents1.png"/>'+item['total_bonus']+'</p>'+
                                '<p class="time"><img src="img/icon_PopularEvents2.png"/>'+item['apply_start_time']+'报名开始</p>'+
                                '<p class="number"><img src="img/icon_PopularEvents3.png"/>'+item['has_team']+'/'+item['max_team']+'</p>'+
